...
def mutually_exclusive_plant_charac(data, new_col="allstats", old_col="inv", exclude="X"):
    """Removes all rows from a data frame where the value of old_col is equal to exclude.

    :param data: A data frame
    :type data:pd.DataFrame
    :param new_col:The name of the column once the data has been excluded.
    :type new_col: str
    :param old_col: The name of the column that contains the value to exclude.
    :type old_col: str
...
def a_simple_formatted_table(ax, data, colLabels=[], a_color="black", colWidths=[], bbox=[], **kwargs):
    ...
    separated from table body by a thicker black line.
...
def a_stacked_bar_chart(ax, bars, xaxis=[], totals={}, palette={}, **kwargs):
    ...
    if totals:
        # this is a separate independent stack
        # its value does not affect the value of <bottom>
